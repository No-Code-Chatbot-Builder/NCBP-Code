...
import { ModeToggle } from "@/app/_components/global/mode-toggle";
...
const ChatBot = () => {
  ...
  const workspaceName = searchParams.get('workspaceId')!;
  ...
  const URL = `http://localhost:3004?workspaceId=${workspaceName}&assistantId=${assistantId}`;
  ...
  useEffect(() => {
    ...
    socketRef.current.on("gptResponse", (data: any) => {
      ...
      if (data.event === "textCreated") {
        ...
        dispatch(updateMessage({ assistantId: assistantId, message: data.data }));
        setInputDisabled(false);
      }
    ...
    <div className=" rounded-lg ring-4 ring-ring relative bg-background w-full h-full pt-24">
      ...
      <div className="rounded-md absolute bg-background bottom-0 w-full max-w-3xl px-10 pb-10">
        ...
        <p className="text-secondary font-medium text-xs mt-2">
          Model: ${assistantId}. Generated Content may be
          incorrect or false.
        </p>
      </div>
    </div>

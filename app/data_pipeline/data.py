...
async def configure_data(user_id, workspace_id, dataset_id):
    try:  
        ...
        if response['Items'][0]['jsonlId'] != '':
            ...
            return {"statuscode": 200, "content": data_id}
        else:
            ...
            await save_data_locally(merged_jsonl_data)
            # with open('local_data.jsonl', 'r') as f:
            #     merged_jsonl_data = f.read()
            data_id = await save_to_db(merged_jsonl_data,user_id, workspace_id,dataset_id)
            return {"statuscode": 200, "content": data_id}

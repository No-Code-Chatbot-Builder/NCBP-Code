...
from app.services.s3.index import save_to_db
...
@app.get("/finetune/health")
async def root():
    return {"message": "Greetings from ML server!!"}
    
#tested
@app.get("/finetune/model/status")
...
#tested
@app.post("/finetune/model/fine-tune")
async def fine_tune_model(request: Request, fine_tune_request: FineTuneRequest):
    ...
    if response.get("statuscode") == 204:
          return {
            ...
            "message" : "No proper data exist in the provided dataset for fine tuning. Try again using correct data"
            }
        ...
        response = await create_fine_tuning_job(file_id=response.get("content"),input=fine_tune_request,user_id=request.state.user.get('id'))

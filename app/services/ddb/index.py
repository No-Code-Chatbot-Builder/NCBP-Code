...
async def save_fine_tuning_job_to_dynamodb(workspace_id, fine_tuning_job, user_id, created_at, instruction, data_id, assistant_name,epochs,lr,batch_size):
    ...
    table.put_item(
        Item={
            ...
            'modelId': '', #here when model training is completed id will be inserted and status will be changed with complete
            'modelName': assistant_name,
            'purpose': instruction,
...
async def update_fine_tuning_job_on_success(workspace_id,job_id,assistant_id):
    items = table.scan(
        FilterExpression= Key('PK').eq(f'WORKSPACE#{workspace_id}') & Key('SK').begins_with('MODEL#') & Key('jobId').eq(job_id)
    )['Items']

...
const CreateDomainForm = ({ assistantId }: CreateDomainFormProps) => {
  ...
  const handleAddSite = async (e: FormEvent) => {
    ...
    if (newSite && !assistant!.allowedDomain?.includes(newSite)) {
      form.setValue('site', '');
      try {
        const response = await addDomain(currentWorkspaceName!, assistant!.id, newSite);
        dispatch(
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem className="flex-1">
                ...
                <FormControl>
                  <Input placeholder="Enter the assistant name" {...field} value={assistant!.name} readOnly />
                </FormControl>
                ...
                <FormControl>
                  <div className="flex gap-2">
                    <Input
                      ...
                      {...field}
                    />
                    <Button onClick={(e) => handleAddSite(e)} disabled={!field.value!.trim()}>Add Site</Button>
                  </div>
                </FormControl>
                <FormMessage className="text-red-600 text-xs px-1" />
              </FormItem>

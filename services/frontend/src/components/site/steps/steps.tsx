...
const Steps = ({ imageLink, title, description, className }: Props) => {
  return (
    <div
      className={cn(
        "lg:grid lg:grid-cols-2 gap-20 m-10 lg:place-items-center flex flex-col-reverse w-full justify-center items-center overflow-hidden"
      )}
    >
      <div
        className={cn(
      ...
      >
        ...
        {/* <div className="w-full h-full  bg-gradient-to-r from-sky-500/50 to-indigo-500/50 rounded-3xl" /> */}
      </div>
      <div className="flex flex-col gap-4">
        <h1 className="text-3xl font-bold">{title}</h1>
        <p className="leading-normal text-muted-foreground w-full">
          {description}
        </p>
      </div>
    </div>
